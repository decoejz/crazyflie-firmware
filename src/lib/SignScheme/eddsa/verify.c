...
int verify_eddsa(uint8_t *msg_raw, uint8_t *msg_signed, int total_len, pki_t public_key)
{
    ...
    if (msg_len < 0 || sigma_len <= 0 || (msg_len + sigma_len + EDDSA_SIGN_HEADER_SIZE) > total_len) {
        log_error("Validation ERROR - Invalid message format");
        return -1;
    }
    ...
    {
        log_error("Validation ERROR - EVP_DigestVerifyInit failed");
    ...
    {
        char buffer[64];
        snprintf(buffer, sizeof(buffer), "Validation ERROR - code %d", res);
        log_error(buffer);
        return res;
    }
}